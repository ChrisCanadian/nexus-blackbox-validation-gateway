...
async def test_gateway_end_to_end_and_never_returns_provider_key():
    ...
    app = create_app(router_admin=router, target_client=FakeTarget())
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://gateway") as client:
        ...
        turn = await client.post(f"/v1/sandboxes/{sandbox_id}/turns", json={"message": "hello"})
        assert turn.status_code == 200
        body = turn.json()
        assert body["response"] == "opaque:hello"
